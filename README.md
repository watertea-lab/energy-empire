...
一款以「能源與能源類股」為主題的虛擬交易網頁遊戲。玩家以 **新台幣 NT$10,000,000** 起始資金,在可旋轉的 3D 地球上檢視各國能源結構、配置能源投資組合,並以**真實市場數據**買賣能源類股、追蹤資產淨值與報酬。

> 核心特色:串接真實 API 數據並**安全保管金鑰**(後端代理 + 環境變數);全遊戲**統一以新台幣計價**;股價資料具備**優雅降級(fallback)**機制,主來源失效時自動切換備援。
...
- **能源市場(綠能策略)**:配置太陽能、石油、煤、核能、風能占比。**再生能源(太陽+風)占比越高,每回合被動收入越高**;面板即時顯示「再生占比、綠能加成、累計投入、回本回合」,讓玩家評估投資是否划算。
...
- **即時數據儀表板**:整合油價、天氣、太陽輻射、空氣品質等公開資料。
...
**設計原則**:只有需要保護金鑰的來源(Finnhub)走後端代理;其餘免金鑰且支援 CORS 的公開資料,直接由前端取得,以降低複雜度。Yahoo Finance 雖免金鑰,但為統一介面、避免 CORS、並作為美股後備,亦以後端代理提供。
...
│       └── quote-intl.js            # Yahoo Finance 代理(國際 / A 股,兼作美股後備)
└── .github/workflows/
...
| `/api/quote-intl` | GET | `symbol`(如 `601857.SS`、`ORSTED.CO`、`300750.SZ`、`XOM`) | 代理 Yahoo Finance,回傳與 Finnhub 一致的 `{ c, pc }`;亦作為美股後備來源 |
...
- **資料持久化**:目前遊戲狀態存於記憶體,重新整理會重置;可改用 `localStorage` 保存資金與投資組合。
